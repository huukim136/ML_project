...
def save_checkpoint(model, optimizer_main, learning_rate, iteration, filepath):
...
def validate(model, criterion, valset, iteration, batch_size, n_gpus,
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            losses, acces, l_main = criterion(y_pred, y, False)
            if distributed_run:
                reduced_val_losses = []
                reduced_val_acces = []
        ...
        val_loss_tts = val_loss_tts / i

        reduced_val_tts_acces = reduced_val_tts_acces / i

        reduced_val_tts_losses = reduced_val_tts_losses / i
...
def train(output_directory, log_directory, checkpoint_path, warm_start, n_gpus,
    ...
    parameters_main = model.grouped_parameters()
        ...
        for i, batch in enumerate(train_loader):
            ...
            if not math.isnan(redl_main) and rank == 0:
                ...
                print(("Train {} {} {:.6f} Grad Norm {:.6f} {:.2f}s/it".format(
                    task, iteration, redl_main, grad_norm_main, duration)))
                logger.log_training(
                    redl_main, reduced_losses, reduced_acces, grad_norm_main, learning_rate, duration, iteration)
            ...
            if (iteration % hparams.iters_per_checkpoint == 0):
                ...
                if rank == 0:
                    ...
                    save_checkpoint(model, optimizer_main, learning_rate, iteration,
                                    checkpoint_path)
